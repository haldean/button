...
public class LightsService extends IntentService {
	...
	// Percentage change that triggers an update
	...
	private void establishConnection() {
		try {
			...
			Log.e("LightsService",
					"Couldn't establish sockets; falling back to HTTP API.", e);
			sock = null;
		}
	}
	...
	protected void onHandleIntent(Intent intent) {
		...
		hostAddr = sp.getHostAddress();
		socketPort = sp.getHostSocketPort();
		httpPort = sp.getHostHttpPort();
		...
		boolean hueShouldChange = (Math.abs(hue - lastHueUpdate)
				/ lastHueUpdate > MIN_DIFF || Double.isNaN(lastHueUpdate));
		boolean valueShouldChange = (Math.abs(value - lastValueUpdate)
				/ lastValueUpdate > MIN_DIFF || Double.isNaN(lastValueUpdate));
		...
		if (hueShouldChange || valueShouldChange) {
			...
			if (sock != null) {
				byte[] buffer = (Double.toString(hue) + ",1,"
				...
				try {
					...
					Log.e("LightsService",
							"Couldn't write to open socket; falling back to HTTP API.",
							e);
					sock = null;
				}
				...
				try {
					URL updateUrl = new URL("http://" + hostAddr + ":"
							+ httpPort + "/hsvapi?h=" + Double.toString(hue)
							+ "&s=1&v=" + Double.toString(value));

					HttpURLConnection conn = (HttpURLConnection) updateUrl
							.openConnection();
